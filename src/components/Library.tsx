...
export default function Library({ settings, plans, onUpdateSettings, onUpdatePlan }: LibraryProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-app">
      ...
      <AnimatePresence>
        {editingTask && (
          <div className="fixed inset-0 z-[100] flex items-end justify-center overflow-hidden sm:items-center bg-black/40 backdrop-blur-sm">
            <motion.div 
              initial={{ opacity: 0, y: '100%' }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: '100%' }}
              transition={{ type: 'spring', damping: 32, stiffness: 380, mass: 0.85 }}
              className="bg-app w-full sm:max-w-md rounded-t-[32px] sm:rounded-[32px] rounded-b-none shadow-2xl relative max-h-[88dvh] sm:max-h-[90vh] flex flex-col overflow-hidden"
            >
              <button 
                onClick={() => setEditingTask(null)}
                className="absolute top-6 right-6 p-2 bg-app rounded-full text-secondary hover:text-primary transition-colors z-10"
              >
                <X className="w-5 h-5" />
              </button>
